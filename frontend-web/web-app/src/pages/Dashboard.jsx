    ...
    const fetchDashboardData = async () => {
      try {
        ...
        setTaskStatusData([
          ...
          { name: 'Overdue', value: 5     </PageLayout>
  );
}
        ]);
        setEmployeeDistributionData([
          ...
          { name: 'Employee', value: 142     </PageLayout>
  );
}
        ]);
        setRecentActivities([
          ...
          { type: 'task', title: 'Update database schema', description: 'Add new fields to employee table', status: 'pending', timestamp: new Date().toISOString()     </PageLayout>
  );
}
        ]);
      } finally {
              ...
              <div key={index} className="flex items-start space-x-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex-1">
                  <Typography variant="small" color="blue-gray" className="font-medium">
                    {activity.title    </PageLayout>
  );
}
                  </Typography>
                  <Typography variant="small" color="gray">
                    {activity.description    </PageLayout>
  );
}
                  </Typography>
                  <Typography variant="small" color="gray">
                    Status: {activity.status} • {new Date(activity.timestamp).toLocaleDateString()    </PageLayout>
  );
}
                  </Typography>
                </Card>
