...
from app.schemas.ai import (
    AIQueryRequest, AIQueryResponse, RiskAssessment, PolicyDSL,
    ApprovalRequest, ApprovalDecision, AutoRestriction
)
...
def ai_query(
    ...
    try:
        ...
        decision, reason, evaluation_context = RiskGovernor.evaluate_request(
            db, current_user, request.capability.value, {"query_text": request.query_text}
        )
...
def get_risk_assessment(
    ...
    try:
        ...
        recent_violations = TrustService.get_recent_violations(db, current_user.id, hours=24)
...
def update_policies(
    ...
    try:
        ...
        for rule in new_rules:
            ...
            if not is_valid:
                raise HTTPException(status_code=400, detail=f"Invalid rule {rule.rule_id}: {error}")

        # Add rules to engine
        for rule in new_rules:
            policy_engine.add_rule(rule)

        return {"message": f"Added {len(new_rules)} policy rules", "rules": [r.rule_id for r in new_rules]}
...
def lift_restriction(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to lift restriction: {str(e)}")
