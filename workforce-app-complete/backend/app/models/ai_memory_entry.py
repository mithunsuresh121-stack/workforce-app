...
class AIMemoryEntry(Base):
    ...
    purpose = Column(String(100), nullable=False, index=True)  # Purpose of memory storage
    ...
    expires_at = Column(DateTime, nullable=True, index=True)  # Computed from created_at + ttl
    hash_chain_ref = Column(String(64), nullable=True, index=True)  # Reference to audit chain hash
    ...
    __table_args__ = (
        Index('idx_ai_memory_tenant_key', 'company_id', 'department_id', 'team_id', 'key'),
    ...
    def is_accessible_by(self, user_company_id: int, user_department_id: int = None,
                        user_team_id: int = None, user_role: str = None) -> bool:
