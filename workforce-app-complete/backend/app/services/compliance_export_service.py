...
class ComplianceExportService:
    @staticmethod
    def generate_compliance_report(db: Session, request: ComplianceExportRequest) -> dict:
        ...
        if request.include_trust_history:
            report_data["trust_score_history"] = ComplianceExportService._get_trust_history(
                db, request.company_id, start_date, end_date
            )

        # Generate summary statistics
        report_data["violation_summary"] = ComplianceExportService._calculate_violation_summary(
    ...
    @staticmethod
    def _get_detailed_logs(db: Session, company_id: int, start_date: datetime, end_date: datetime) -> list:
    ...
    @staticmethod
    def _get_trust_history(db: Session, company_id: int, start_date: datetime, end_date: datetime) -> dict:
    ...
    def generate_pdf_report(report_data: dict) -> BytesIO:
        ...
        story.append(Paragraph(f"Period: {meta['date_range']['start']} to {meta['date_range']['end']}", styles['Normal']))
