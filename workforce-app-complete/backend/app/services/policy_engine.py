...
from app.schemas.ai import AIPolicyType, AIPolicySeverity
...
class PolicyRule:
    ...
    def __init__(self, rule_id: str, conditions: Dict[str, Any], actions: List[str], priority: int = 100):
    ...
    def _evaluate_condition(self, context_value: Any, condition: Dict[str, Any]) -> bool:
...
class PolicyEngine:
    ...
    def _load_default_policies(self):
        ...
        self.add_rule(PolicyRule(
            ...
            conditions={
                "is_weekend": True,
                "capability": {"op": "in", "value": ["READ_COMPANY_DATA", "GENERATE_SUMMARY"]}
            },
            actions=[self.ACTION_CHALLENGE],
            priority=25
        ))
            ...
            try:
                ...
                for cond in conditions_part.split(','):
                    ...
                    if '>' in cond:
                        ...
                        conditions[key.strip()] = {"op": "gt", "value": float(value.strip())}
                    elif '<' in cond:
                        key, value = cond.split('<', 1)
                        conditions[key.strip()] = {"op": "lt", "value": float(value.strip())}
                    elif '=' in cond:
                        key, value = cond.split('=', 1)
    ...
    def validate_policy_rule(self, rule: PolicyRule) -> Tuple[bool, str]:
        ...
        valid_actions = [self.ACTION_ALLOW, self.ACTION_DENY, self.ACTION_CHALLENGE, self.ACTION_ESCALATE]
