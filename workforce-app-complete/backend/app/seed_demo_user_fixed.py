...
def seed_demo_user(db: Session):
    ...
    if super_admin:
        ...
        existing_profile = db.query(db.model.employee_profile).filter_by(user_id=super_admin.id).first()
    ...
    if demo_user:
        # Check if profile already exists
        existing_profile = db.query(db.model.employee_profile).filter_by(user_id=demo_user.id).first()
        if not existing_profile:
            create_employee_profile(
                db=db,
