...
def upgrade():
    ...
    op.add_column('leaves', sa.Column('created_at', sa.DateTime(), nullable=True, default=func.now()))
    op.add_column('leaves', sa.Column('updated_at', sa.DateTime(), nullable=True, default=func.now(), onupdate=func.now()))

    # Add created_at and updated_at columns to shifts table
    op.add_column('shifts', sa.Column('created_at', sa.DateTime(), nullable=True, default=func.now()))
    op.add_column('shifts', sa.Column('updated_at', sa.DateTime(), nullable=True, default=func.now(), onupdate=func.now()))

    # Set default values for existing records
    op.execute("UPDATE leaves SET created_at = NOW(), updated_at = NOW() WHERE created_at IS NULL")
    op.execute("UPDATE shifts SET created_at = NOW(), updated_at = NOW() WHERE created_at IS NULL")
