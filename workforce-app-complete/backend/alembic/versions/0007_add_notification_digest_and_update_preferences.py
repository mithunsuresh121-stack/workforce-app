...
def upgrade():
    ...
    op.add_column('notification_preferences', sa.Column('mute_all', sa.Boolean(), nullable=True))
    op.add_column('notification_preferences', sa.Column('digest_mode', sa.String(length=20), nullable=True))
    op.add_column('notification_preferences', sa.Column('push_enabled', sa.Boolean(), nullable=True))
    op.add_column('notification_preferences', sa.Column('notification_types', sa.JSON(), nullable=True))

    # Update existing records with default values
    op.execute("UPDATE notification_preferences SET mute_all = false WHERE mute_all IS NULL")
    op.execute("UPDATE notification_preferences SET digest_mode = 'immediate' WHERE digest_mode IS NULL")
    op.execute("UPDATE notification_preferences SET push_enabled = true WHERE push_enabled IS NULL")
    op.execute("UPDATE notification_preferences SET notification_types = '{\"TASK_ASSIGNED\": true, \"SHIFT_SCHEDULED\": true, \"SYSTEM_MESSAGE\": true, \"ADMIN_MESSAGE\": true}' WHERE notification_types IS NULL")
...
def downgrade():
    ...
    op.add_column('notification_preferences', sa.Column('preferences', sa.JSON(), nullable=True))
