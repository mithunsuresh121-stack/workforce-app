...
import { Typography, Card, CardBody, Skeleton, Button } from '@material-tailwind/react';
import {
  BuildingOfficeIcon,
  PhoneIcon,
  CalendarDaysIcon,
...
const ProfileDetails = ({ profile, view = 'overview' }) => {
  // Add toggle states for each section
  ...
  const renderSectionHeader = (title, icon: Icon, isExpanded, onToggle, hasData) => {
    if (!hasData) return null;

    return (
      <div className="flex items-center justify-between mb-4">
        <Typography variant="h5" color="blue-gray" className="flex items-center gap-3 font-bold text-left cursor-pointer" onClick={() => {}}>
        ...
        <Button
          ...
          size="sm"
          onClick={onToggle}
          className="flex items-center gap-1 text-gray-600 hover:text-blue-600 p-1"
        >
          {isExpanded ? (
            <>
  ...
  const renderOverview = () => (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {hasWorkData && (
          <div>
            {renderSectionHeader('Work Information', BriefcaseIcon, workExpanded, () => setWorkExpanded(!workExpanded), hasWorkData)}
            {workExpanded && (
        ...
        {hasContactData && (
          <div>
            {renderSectionHeader('Contact Information', PhoneIcon, contactExpanded, () => setContactExpanded(!contactExpanded), hasContactData)}
            {contactExpanded && (
              <div className="space-y-3">
      ...
      {(hasPersonalData || contactFields.filter(field => field.value !== 'Not set').slice(2).length > 0) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {hasPersonalData && (
            <div>
              {renderSectionHeader('Personal Information', UserIcon, personalExpanded, () => setPersonalExpanded(!personalExpanded), hasPersonalData)}
              {personalExpanded && (
                <div className="space-y-3">
