...
def test_shift_timestamp_tracking(base_url, superadmin_jwt, auth_headers):
    ...
    shift_date = (datetime.now() + timedelta(days=20)).strftime("%Y-%m-%d")

    payload = {
        "employee_id": 3,
        "shift_name": "Timestamp Test Shift",
    ...
    r = requests.put(f"{base_url}/shifts/{shift_id}/status", json={"status": "Completed"}, headers=auth_headers(superadmin_jwt))
    assert r.status_code == 200
...
def test_shift_business_logic(base_url, superadmin_jwt, auth_headers):
    ...
    second_shift_payload = {
        ...
        "shift_date": shift_date,
        "shift_time": "14:00-22:00"  # Overlaps with first shift
    }
...
def test_shift_data_integrity(base_url, superadmin_jwt, auth_headers):
    ...
    shift_date = (datetime.now() + timedelta(days=15)).strftime("%Y-%m-%d")

    payload = {
        "employee_id": 3,
        "shift_name": "Duplicate Test Shift",
        "shift_date": shift_date,
        "shift_time": "09:00-17:00"
    }
