...
def upgrade() -> None:
    ...
    op.create_foreign_key(None, 'channels', 'company_departments', ['department_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key(None, 'channels', 'company_teams', ['team_id'], ['id'], ondelete='CASCADE')
    ...
    op.create_foreign_key(None, 'meetings', 'company_departments', ['department_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key(None, 'meetings', 'company_teams', ['team_id'], ['id'], ondelete='CASCADE')
    ...
    op.execute("UPDATE users SET role = 'DEPARTMENT_ADMIN' WHERE role = 'Manager' OR role = 'MANAGER'")
    op.execute("UPDATE users SET role = 'EMPLOYEE' WHERE role = 'Employee'")
    # Set any remaining invalid roles to EMPLOYEE
    op.execute("UPDATE users SET role = 'EMPLOYEE' WHERE role NOT IN ('SUPERADMIN', 'COMPANY_ADMIN', 'DEPARTMENT_ADMIN', 'TEAM_LEAD', 'EMPLOYEE')")

    # Create the enum type first
    userrole_enum = sa.Enum('SUPERADMIN', 'COMPANY_ADMIN', 'DEPARTMENT_ADMIN', 'TEAM_LEAD', 'EMPLOYEE', name='userrole')
    ...
    op.create_foreign_key(None, 'users', 'company_departments', ['department_id'], ['id'])
...
def downgrade() -> None:
    ...
    op.drop_constraint(None, 'users', type_='foreignkey')
    op.alter_column('users', 'role',
               existing_type=sa.Enum('SUPERADMIN', 'COMPANY_ADMIN', 'DEPARTMENT_ADMIN', 'TEAM_LEAD', 'EMPLOYEE', name='userrole'),
               type_=sa.VARCHAR(),
               existing_nullable=True)
    ...
    sa.Column('allow_employee_registration', sa.BOOLEAN(), autoincrement=False, nullable=True),
    sa.Column('require_manager_approval', sa.BOOLEAN(), autoincrement=False, nullable=True),
    sa.Column('enable_notifications', sa.BOOLEAN(), autoincrement=False, nullable=True),
    ...
    sa.ForeignKeyConstraint(['company_id'], ['companies.id'], name=op.f('company_settings_company_id_fkey'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('company_settings_pkey')),
    sa.UniqueConstraint('company_id', name=op.f('company_settings_company_id_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
