...
def test_company(db):
    ...
    company = Company(
        name=unique_name, domain=unique_domain, contact_email="test@testco.com"
    )
    db.add(company)
    db.commit()
    db.refresh(company)
...
def test_company2(db):
    company = Company(
        name="Test Company 2", domain="testco2.com", contact_email="test2@testco2.com"
    )
    db.add(company)
    db.commit()
    db.refresh(company)
...
def test_create_user_preferences(db: Session, test_user, test_company):
    ...
    prefs = create_user_preferences(
        db, user_id=test_user.id, company_id=test_company.id, preferences=custom_prefs
    )
...
def test_update_user_preferences(db: Session, test_user, test_company):
    ...
    prefs = create_user_preferences(
        db, user_id=test_user.id, company_id=test_company.id
    )
    ...
    result = update_user_preferences(
        db, user_id=test_user.id, company_id=test_company.id, preferences=updated_prefs
    )
...
def test_delete_user_preferences(db: Session, test_user, test_company):
    ...
    # Delete them
    success = delete_user_preferences(
        db, user_id=test_user.id, company_id=test_company.id
    )
    ...
    # Verify deletion
    deleted_prefs = (
        db.query(NotificationPreferences).filter_by(user_id=test_user.id).first()
    )
    assert deleted_prefs is None


def test_get_or_create_user_preferences(db: Session, test_user, test_company):
    prefs = get_or_create_user_preferences(
        db=db, user_id=test_user.id, company_id=test_company.id
    )
    assert prefs.user_id == test_user.id
    assert prefs.company_id == test_company.id
...
def test_should_send_notification(db: Session, test_user, test_company):
    ...
    should_send = should_send_push_notification(
        db, user_id=999, company_id=78, notification_type="TASK_ASSIGNED"
    )
