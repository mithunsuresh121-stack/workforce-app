...
def test_profile_error_handling():
    ...
    with patch(
        "backend.app.routers.profile_final.get_employee_profile_by_user_id"
    ) as mock_get_profile:
        # Simulate database permission error
        mock_get_profile.side_effect = Exception(
            "permission denied for table employee_profiles"
        )

        with patch(
            "backend.app.routers.profile_final.create_employee_profile"
