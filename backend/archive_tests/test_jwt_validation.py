...
def test_jwt_structure_and_claims():
    ...
    try:
        ...
        try:
            ...
            ).total_seconds() > 3600 * 24:  # More than 24 hours
                print(f"✗ Token expiry time too long: {exp_time}")
                return False
            else:
                print(f"✓ Valid expiry time: {exp_time}")
                print(
                    f"✓ Token expires in: {(exp_time - current_time).total_seconds() / 60:.1f} minutes"
                )

            # Test token validation with protected endpoint
            headers = {"Authorization": f"Bearer {access_token}"}
            companies_response = requests.get(
                f"{BASE_URL}/companies/1", headers=headers
            )
            ...
            if companies_response.status_code == 200:
                ...
                print(
                    f"✗ Token validation failed: {companies_response.status_code} {companies_response.text}"
                )
                return False
