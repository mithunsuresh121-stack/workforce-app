...
def test_timestamp_tracking():
    ...
    response = requests.put(
        f"{BASE_URL}/leaves/{leave_id}/status", headers=headers, json=status_update
    )

    if response.status_code != 200:
        print(
            f"❌ Failed to update leave status: {response.status_code} - {response.text}"
        )
        return False
...
def test_status_values():
    ...
    for status in valid_statuses:
        ...
        response = requests.post(
            f"{BASE_URL}/leaves/", headers=headers, json=leave_data
        )
        if response.status_code == 201:
            created_leave = response.json()
            leave_id = created_leave.get("id")
            ...
            print(
                f"❌ Status '{status}' rejected: {response.status_code} - {response.text}"
            )
            return False
    ...
    response = requests.post(
        f"{BASE_URL}/leaves/", headers=headers, json=invalid_leave_data
    )
...
def test_multi_tenant_isolation():
    ...
    response = requests.post(
        f"{BASE_URL}/leaves/", headers=headers_manager, json=leave_data_tenant1
    )
    if response.status_code != 201:
    ...
    response = requests.post(
        f"{BASE_URL}/leaves/", headers=headers_manager, json=leave_data_tenant2
    )
    if response.status_code != 201:
    ...
    if response.status_code == 200:
        ...
        tenant1_leaves = [
            leave for leave in all_leaves if leave.get("tenant_id") == "1"
        ]
        tenant2_leaves = [
            leave for leave in all_leaves if leave.get("tenant_id") == "2"
        ]
...
def test_shift_timestamp_tracking():
    ...
    response = requests.put(
        f"{BASE_URL}/shifts/{shift_id}", headers=headers, json=updated_shift_data
    )
...
def main():
    print(
        "🚀 Starting validation of timestamp tracking, status values, and multi-tenant isolation"
    )
    print("=" * 80)
    ...
    print("=" * 80)
    print(
        f"Timestamp tracking (leaves): {'✅ PASS' if timestamp_test_passed else '❌ FAIL'}"
    )
    ...
    print(
        f"Timestamp tracking (shifts): {'✅ PASS' if shift_timestamp_test_passed else '❌ FAIL'}"
    )

    all_passed = all(
        [
            timestamp_test_passed,
            status_test_passed,
            tenant_test_passed,
            shift_timestamp_test_passed,
        ]
    )
    ...
    if all_passed:
        ...
        print(
            "✅ Timestamp tracking, status values, and multi-tenant isolation are working correctly"
        )
    else:
        print("\n⚠️  SOME VALIDATION TESTS FAILED!")
        print(
            "❌ Issues found with timestamp tracking, status values, or multi-tenant isolation"
        )
