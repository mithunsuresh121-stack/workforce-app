...
def test_company_cascade_delete():
    ...
    try:
        ...
        assert (
            session.query(MessageReaction).filter_by(id=reaction.id).first() is not None
        )
...
def test_timestamp_updates():
    ...
    try:
        ...
        if initial_last_active != updated_last_active:
            ...
            print(
                "⚠ User last_active timestamp may not be updating (could be same millisecond)"
            )
        ...
        if initial_last_message_at != updated_last_message_at:
            ...
            print(
                "⚠ Channel last_message_at timestamp may not be updating (could be same millisecond)"
            )
