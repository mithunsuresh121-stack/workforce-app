...
def get_auth_token():
    response = client.post(
        "/api/auth/login", json={"email": "demo@company.com", "password": "password123"}
    )
...
def get_super_admin_token():
    response = client.post(
        "/api/auth/login", json={"email": "admin@app.com", "password": "supersecure123"}
    )
    assert response.status_code == 200
    return response.json()["access_token"]
...
def test_approve_profile_request():
    ...
    response = client.put(
        f"/api/profile/requests/{request_id}/approve",
...
def test_reject_profile_request():
    ...
    admin_headers = {"Authorization": f"Bearer {admin_token}"}
    response = client.put(
        f"/api/profile/requests/{request_id}/reject",
