...
class MeetingService:
    ...
    def create_meeting(
        ...
        meeting = create_meeting(
            db, title, organizer_id, company_id, start_time, end_time
        )

        # Add organizer as participant
        add_participant_to_meeting(
            db, meeting.id, organizer_id, ParticipantRole.ORGANIZER
        )

        # Add other participants
        for participant_id in participant_ids:
            add_participant_to_meeting(
                db, meeting.id, participant_id, ParticipantRole.PARTICIPANT
            )
    ...
    def join_meeting(
        self, db: Session, meeting_id: int, user_id: int
    ) -> MeetingParticipant:
        """User joins the meeting"""
        participant = update_participant_join_time(
            db, meeting_id, user_id, datetime.utcnow()
        )
        ...
        if (
            ...
            and meeting.start_time <= datetime.utcnow()
        ):
    ...
    def get_meetings_for_user(
        ...
        meetings = (
            ...
            .filter(
                Meeting.company_id == company_id, MeetingParticipant.user_id == user_id
            )
    ...
    async def get_online_participants(
        self, meeting_id: int, company_id: int
    ) -> List[int]:
        ...
        if participant and not participant.leave_time:
            ...
            if meeting:
                ...
                await redis_service.publish_event(
                    ...
                    {"type": "user_left", "user_id": user_id, "meeting_id": meeting_id},
                )
