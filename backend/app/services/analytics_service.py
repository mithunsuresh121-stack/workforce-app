...
class AnalyticsService:
    ...
    def get_channel_stats(db: Session, company_id: int = None) -> dict:
        ...
        active_channels = query.filter(Channel.last_message_at >= seven_days_ago).count()

        # Average members per channel (using ChannelMember)
        from app.models.channels import ChannelMember
        from sqlalchemy import func
        subq_members = db.query(
            func.count(ChannelMember.user_id).label('member_count')
        ).join(
            Channel, ChannelMember.channel_id == Channel.id
        ).filter(
            Channel.company_id == company_id if company_id else True
        ).group_by(Channel.id).subquery()
    ...
    def get_meeting_stats(db: Session, company_id: int = None) -> dict:
        ...
        from sqlalchemy import func
        subq_participants = db.query(
            func.count(MeetingParticipant.user_id).label('participant_count')
        ).join(
            Meeting, MeetingParticipant.meeting_id == Meeting.id
        ).filter(
            Meeting.company_id == company_id if company_id else True
        ).group_by(Meeting.id).subquery()
        avg_participants = db.query(func.avg(subq_participants.c.participant_count)).scalar() or 0
    ...
    def get_audit_stats(db: Session, company_id: int = None) -> dict:
        ...
        top_users_by_activity = [{'user_id': uid, 'event_count': count} for uid, count in top_users]
