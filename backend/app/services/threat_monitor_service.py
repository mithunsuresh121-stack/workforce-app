...
from app.schemas.ai import AIPolicySeverity, WebhookAlertConfig
...
class ThreatMonitorService:
    ...
    def get_live_violations(
        db: Session, company_id: int = None, limit: int = 50
    ) -> list:
        """Get live feed of recent AI violations and anomalies"""
        query = (
            db.query(AuditLog)
            .filter(AuditLog.event_type.like("SECURITY_AI_%"))
    ...
    def get_risk_heatmap(
        db: Session, company_id: int = None, time_range_hours: int = 24
    ) -> dict:
    ...
    def check_threshold_alerts(db: Session, company_id: int = None) -> list:
        ...
        if company_id:
            high_severity_count = high_severity_count.filter(
        ...
        user_violations = (
            db.query(AuditLog.user_id, func.count(AuditLog.id).label("violation_count"))
            .filter(
                AuditLog.event_type.like("SECURITY_AI_%"),
                AuditLog.created_at >= cutoff_time,
            )
    ...
    def send_webhook_alert(
        webhook_config: WebhookAlertConfig, alert_data: dict
    ) -> bool:
        ...
        try:
            ...
            response = requests.post(
                webhook_config.url, json=payload, headers=headers, timeout=10
            )
            ...
            logger.error(
                "Webhook alert exception", url=webhook_config.url, error=str(e)
            )
            return False
    ...
    def auto_lock_high_risk_user(db: Session, user_id: int, risk_score: int) -> bool:
        ...
        if risk_score >= 90:  # Very high risk threshold
            SecurityService.lock_account(
                db, user_id, "Auto-locked due to high-risk AI violations"
            )
            ...
            logger.critical(
                "User auto-locked for high-risk violations",
                user_id=user_id,
