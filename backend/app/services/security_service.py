...
class SecurityService:
    ...
    @staticmethod
    def check_anomaly_rules(db: Session, user_id: int, event_type: SecurityEvent, details: dict = None) -> Optional[SecuritySeverity]:
        ...
        if event_type == SecurityEvent.AUTH_FAILURE:
            ...
            recent_failures = db.query(AuditLog).filter(
                ...
                AuditLog.event_type == f"SECURITY_{SecurityEvent.AUTH_FAILURE.value}",
                AuditLog.created_at >= datetime.utcnow() - timedelta(hours=1)
            ).count()

            if recent_failures >= SecurityService.MAX_FAILED_LOGINS - 1:  # -1 because current event not yet counted
                SecurityService.lock_account(db, user_id, f"Too many failed login attempts ({recent_failures + 1})")
                return SecuritySeverity.HIGH

        elif event_type == SecurityEvent.CROSS_ORG_ACCESS:
            # Check for repeated cross-org attempts
            recent_cross_org = db.query(AuditLog).filter(
                AuditLog.user_id == user_id,
                AuditLog.event_type == f"SECURITY_{SecurityEvent.CROSS_ORG_ACCESS.value}",
                AuditLog.created_at >= datetime.utcnow() - timedelta(hours=1)
            ).count()

            if recent_cross_org >= SecurityService.CROSS_ORG_THRESHOLD - 1:  # -1 because current event not yet counted
                return SecuritySeverity.HIGH
    ...
    def lock_account(db: Session, user_id: int, reason: str):
        ...
        if user:
            ...
            SecurityService.log_security_event(
                ...
                details={"reason": reason, "locked_until": user.locked_until.isoformat()}
            )
    ...
    @staticmethod
    def get_security_alerts(db: Session, company_id: Optional[int] = None, limit: int = 50) -> List[Dict]:
