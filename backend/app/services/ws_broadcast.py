...
class WSManager:
    def __init__(self):
        self.active_connections: Dict[int, Set[WebSocket]] = {}  # channel_id -> set(WebSocket)
    ...
    async def disconnect(self, channel_id: int, user_id: int, websocket: WebSocket):
        ...
        logger.info("User disconnected from channel", user_id=user_id, channel_id=channel_id)
    ...
    async def broadcast(self, channel_id: int, message_json: str):
        ...
        logger.info("Broadcasted message to channel", channel_id=channel_id, message_size=len(message_json))
