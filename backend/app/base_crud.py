...
def get_user_by_email(
    db: Session, email: str, company_id: Optional[int] = None
) -> Optional[User]:
...
def list_employee_profiles_by_company(db: Session, company_id: int):
    return (
        db.query(EmployeeProfile).filter(EmployeeProfile.company_id == company_id).all()
    )
...
def get_employee_profile_by_user_id(db: Session, user_id: int, company_id: int):
    return (
        ...
        .filter(
            EmployeeProfile.user_id == user_id, EmployeeProfile.company_id == company_id
        )
        .first()
    )
...
def list_profile_update_requests_by_user(db: Session, user_id: int):
    return (
        db.query(ProfileUpdateRequest)
        .filter(ProfileUpdateRequest.user_id == user_id)
        .all()
    )


def update_profile_update_request_status(
    db: Session,
    request_id: int,
    status: str,
    reviewed_by_id: int,
...
from app.models.payroll import Allowance, Bonus, Deduction
from app.models.payroll import Employee as PayrollEmployee
from app.models.payroll import PayrollEntry, PayrollRun, Salary
...
def get_payroll_employee_by_employee_id(db: Session, employee_id: str, tenant_id: str):
    return (
        ...
        .filter(
            ...
            PayrollEmployee.tenant_id == tenant_id,
        )
        .first()
    )
...
def list_payroll_employees_by_tenant(db: Session, tenant_id: str):
    return (
        db.query(PayrollEmployee).filter(PayrollEmployee.tenant_id == tenant_id).all()
    )
...
def list_payroll_entries_by_run(db: Session, payroll_run_id: int):
    return (
        ...
        .filter(PayrollEntry.payroll_run_id == payroll_run_id)
        .all()
    )
...
def create_attendance(
    db: Session, company_id: int, employee_id: int, clock_in_time, notes: str = None
):
...
def get_active_attendance_by_employee(db: Session, employee_id: int):
    return (
        db.query(Attendance)
        .filter(
            Attendance.employee_id == employee_id, Attendance.clock_out_time.is_(None)
        )
        .first()
    )
...
def get_attendance_records(
    db: Session, user_id: int, company_id: int, start_date=None, end_date=None
):
    query = db.query(Attendance).filter(
        Attendance.employee_id == user_id, Attendance.company_id == company_id
    )
...
def create_break(
    db: Session, attendance_id: int, break_start, break_type: str = "lunch"
):
    break_record = Break(
        attendance_id=attendance_id, break_start=break_start, break_type=break_type
    )
    db.add(break_record)
    db.commit()
...
def get_payroll_by_dept(
    db: Session, company_id: int, period: str
) -> List[Dict[str, Any]]:
    """
    ...
    dept_data = (
        ...
        .outerjoin(PayrollEmployee, PayrollEmployee.user_id == EmployeeProfile.user_id)
        .filter(
...
def get_overtime_trend(
    db: Session, company_id: int, period: str, dept_filter: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
    ...
    if period == "weekly":
        ...
        label_expr = func.concat(
            ...
            "-",
            func.lpad(extract("week", Attendance.clock_in_time), 2, "0"),
        )
    else:
        # Last 12 months
        ...
        label_expr = func.concat(
            extract("year", Attendance.clock_in_time),
            "-",
            func.lpad(extract("month", Attendance.clock_in_time), 2, "0"),
        )
    ...
    query = (
        ...
        .outerjoin(EmployeeProfile, EmployeeProfile.user_id == Attendance.employee_id)
        .filter(
            Attendance.company_id == company_id,
            Attendance.clock_in_time >= start_date,
