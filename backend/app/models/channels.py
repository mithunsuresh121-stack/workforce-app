...
class Channel(Base):
    ...
    department_id = Column(
        Integer, ForeignKey("company_departments.id", ondelete="CASCADE"), nullable=True
    )
    team_id = Column(
        Integer, ForeignKey("company_teams.id", ondelete="CASCADE"), nullable=True
    )
    created_by = Column(
    ...
    creator = relationship(
        "User", foreign_keys=[created_by], back_populates="created_channels"
    )
    members = relationship(
        "User", secondary="channel_members", overlaps="channel_memberships"
    )
    messages = relationship(
        "ChatMessage", back_populates="channel", cascade="all, delete-orphan"
    )
    channel_members = relationship(
        "ChannelMember",
...
class ChannelMember(Base):
    ...
    channel = relationship(
        "Channel", back_populates="channel_members", overlaps="members"
    )
    user = relationship(
        "User", back_populates="channel_memberships", overlaps="members"
    )
