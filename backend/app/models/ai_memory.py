from sqlalchemy import Column, String, Integer, Boolean, DateTime, ForeignKey, LargeBinary, Index
...
class AIMemoryEntry(Base):
    ...
    department_id = Column(UUID(as_uuid=True), ForeignKey("company_departments.id"), nullable=True, index=True)
    team_id = Column(UUID(as_uuid=True), ForeignKey("company_teams.id"), nullable=True, index=True)
    ...
    __table_args__ = (
        Index('idx_ai_memory_tenant_scope', 'company_id', 'department_id', 'team_id', 'key'),
