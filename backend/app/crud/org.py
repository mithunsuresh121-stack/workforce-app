...
def create_department(
    db: Session, company_id: int, name: str, created_by: int
) -> CompanyDepartment:
    ...
    department = CompanyDepartment(name=name, company_id=company_id)
    db.add(department)
    db.commit()
    ...
    logger.info(
        "Department created", department_id=department.id, company_id=company_id
    )
    return department
...
def update_department(
    db: Session, department_id: int, name: Optional[str] = None
) -> CompanyDepartment:
...
def create_team(
    db: Session, department_id: int, name: str, created_by: int
) -> CompanyTeam:
    """Create a new team"""
    from app.services.audit_service import AuditService
    ...
    team = CompanyTeam(name=name, department_id=department_id)
    db.add(team)
    db.commit()
    db.refresh(team)
...
def get_teams_by_department(db: Session, department_id: int) -> List[CompanyTeam]:
    ...
    return (
        db.query(CompanyTeam).filter(CompanyTeam.department_id == department_id).all()
    )
