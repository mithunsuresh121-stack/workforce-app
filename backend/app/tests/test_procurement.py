...
class TestProcurementService:
    def test_create_vendor(self, db: Session, test_company: Company):
        vendor_data = VendorCreate(
            name="Test Vendor", contact_email="vendor@test.com", phone="123-456-7890"
        )
    ...
    def test_update_vendor(self, db: Session, test_company: Company):
        vendor_data = VendorCreate(name="Test Vendor", contact_email="vendor@test.com")
        vendor = ProcurementService.create_vendor(db, vendor_data, test_company.id)
        updated_data = VendorCreate(
            name="Updated Vendor", contact_email="updated@test.com"
        )
        updated = ProcurementService.update_vendor(
            db, vendor.id, test_company.id, updated_data
        )
    ...
    def test_create_purchase_order(
        self, db: Session, test_company: Company, test_employee_user: User
    ):
        vendor_data = VendorCreate(name="Test Vendor", contact_email="vendor@test.com")
        vendor = ProcurementService.create_vendor(db, vendor_data, test_company.id)
        po_data = PurchaseOrderCreate(
            vendor_id=vendor.id, item_name="Test Item", quantity=10, amount=100.0
        )
        po = ProcurementService.create_purchase_order(
            db, po_data, test_employee_user.id, test_company.id
        )
    ...
    def test_approve_purchase_order(
        ...
        approved = ProcurementService.approve_purchase_order(
            db, po.id, test_admin_user, test_company.id
        )
    ...
    def test_add_bid_to_purchase_order(
        ...
        updated = ProcurementService.add_bid_to_purchase_order(
            db, po.id, bid_data, test_company.id
        )
    ...
    @pytest.mark.asyncio
    async def test_get_inventory_items_cache_miss(
        self, db: Session, test_company: Company
    ):
        # Mock Redis to simulate cache miss
        with patch.object(
            redis_service, "get", return_value=None
        ) as mock_get, patch.object(redis_service, "setex") as mock_setex:
            item_data = InventoryItemCreate(
                name="Test Item", quantity=100, unit_price=10.0
            )
            await ProcurementService.create_inventory_item(
                db, item_data, test_company.id
            )
    ...
    @pytest.mark.asyncio
    async def test_get_inventory_items_cache_hit(
        self, db: Session, test_company: Company
    ):
        ...
        with patch.object(
            redis_service, "get", return_value=json.dumps(cached_data)
        ) as mock_get:
            items = await ProcurementService.get_inventory_items(db, test_company.id)
            assert len(items) == 1
    ...
    async def test_create_inventory_item(self, db: Session, test_company: Company):
        ...
        item = await ProcurementService.create_inventory_item(
            db, item_data, test_company.id
        )
    ...
    async def test_update_inventory_item(self, db: Session, test_company: Company):
        ...
        updated_data = InventoryItemCreate(
            name="Updated Item", quantity=200, unit_price=15.0
        )
        updated = await ProcurementService.update_inventory_item(
            db, item.id, test_company.id, updated_data
        )
    ...
    async def test_delete_inventory_item(self, db: Session, test_company: Company):
        ...
        success = await ProcurementService.delete_inventory_item(
            db, item.id, test_company.id
        )
        assert success
        items = await ProcurementService.get_inventory_items(db, test_company.id)
        assert len(items) == 0
