...
from app.base_crud import create_company, create_user
...
def department_admin_user(db: Session, test_company):
    dept = create_department(
        db, company_id=test_company.id, name="Test Dept", created_by=1
    )
    user = create_user(
        db,
...
def public_channel(db: Session, test_company):
    return create_channel(
        ...
        company_id=test_company.id,
        created_by=1,
    )  # Assuming creator id=1 for test
...
class TestRBACService:
    ...
    def test_company_admin_checks(self, company_admin_user, test_company):
        ...
        assert RBACService.can_manage_department(
            company_admin_user, test_company.id
        )  # Simplified
    ...
    def test_cross_org_message_block(self, company_admin_user, employee_user):
        ...
        assert not RBACService.can_send_cross_org_message(
            company_admin_user, employee_user
        )

    def test_cross_org_channel_invite_block(
        self, company_admin_user, team_channel, employee_user
    ):
        # Same company - should allow
        assert RBACService.can_invite_to_channel(
            company_admin_user, team_channel, employee_user
        )

        # Different company - should block
        employee_user.company_id = 999
        assert not RBACService.can_invite_to_channel(
            company_admin_user, team_channel, employee_user
        )

    def test_cross_org_meeting_invite_block(
        self, company_admin_user, team_meeting, employee_user
    ):
        # Same company - should allow
        assert RBACService.can_invite_to_meeting(
            company_admin_user, team_meeting, employee_user
        )

        # Different company - should block
        employee_user.company_id = 999
        assert not RBACService.can_invite_to_meeting(
            company_admin_user, team_meeting, employee_user
        )
