...
async def test_message_send_receive(db, test_company):
    ...
    with patch("websockets.sync.client.connect") as mock_connect:
        ...
        mock_ws2.recv.return_value = json.dumps(
            {
                "type": "message",
                "data": {"text": "Hello from user1", "sender_id": user1.id},
            }
        )

        # Verify the message was sent correctly
        mock_ws1.send.assert_called_once_with(
            json.dumps({"type": "message_send", "text": "Hello from user1"})
        )
...
async def test_typing_indicators():
    ...
    try:
        ...
        channel = Channel(
            name="Test Channel", company_id=company.id, created_by=user1.id
        )
        db.add(channel)
        db.commit()
...
async def test_reaction_add():
    ...
    try:
        ...
        # Add reaction
        ws1.send(
            json.dumps({"type": "reaction_add", "message_id": msg["id"], "emoji": "👍"})
        )
