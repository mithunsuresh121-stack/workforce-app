...
from app.metrics import MEETINGS_JOINED_KEY, MESSAGES_SENT_KEY, registry

logger = structlog.get_logger(__name__)

# System Metrics (local to exporter)
cpu_usage = Gauge(
    "workforce_cpu_usage_percent", "Current CPU usage percentage", registry=registry
)
memory_usage = Gauge(
    "workforce_memory_usage_percent",
    "Current memory usage percentage",
    registry=registry,
)
memory_used_bytes = Gauge(
    "workforce_memory_used_bytes", "Memory used in bytes", registry=registry
)
...
api_uptime_seconds = Gauge(
    "workforce_api_uptime_seconds", "API uptime in seconds", registry=registry
)
...
def start_metrics_exporter():
    ...
    redis_thread = threading.Thread(
        target=lambda: asyncio.run(update_redis_metrics_loop()), daemon=True
    )
    redis_thread.start()
        ...
        try:
            ...
            if healthy:
                ...
                try:
                    ...
                    logger.debug(
                        "Could not update persistent counters from Redis", error=str(e)
                    )
