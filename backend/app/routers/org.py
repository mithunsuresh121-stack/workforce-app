...
from app.core.rbac import (RBACService, require_company_admin,
                           require_department_admin, require_team_lead)
...
def create_team(
    ...
    department = (
        db.query(CompanyDepartment)
        .filter(CompanyDepartment.id == team.department_id)
        .first()
    )
    if not department:
        raise HTTPException(
...
def assign_user_org(
    ...
    if assignment.department_id:
        department = (
            db.query(CompanyDepartment)
            .filter(CompanyDepartment.id == assignment.department_id)
            .first()
        )
        if not department:
            raise HTTPException(
    ...
    if assignment.team_id:
        team = (
            db.query(CompanyTeam).filter(CompanyTeam.id == assignment.team_id).first()
        )
        if not team:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Team not found"
