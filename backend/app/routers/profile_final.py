...
def get_my_profile(
    ...
    try:
        ...
        if not profile:
            ...
            if current_user.email == "admin@app.com":
                profile = create_employee_profile(
                    ...
                    company_id=current_user.company_id or 4,  # Default to company ID 4 if None
                    department="IT",
        ...
        # If there's any database error (like permission issues), return a mock profile for demo
        if current_user.email == "admin@app.com":
            return EmployeeProfileOut(
                id=1,
                user_id=current_user.id,
                company_id=current_user.company_id or 4,
                department="IT",
                position="System Administrator",
                phone="+1234567890",
                hire_date="2023-01-01",
