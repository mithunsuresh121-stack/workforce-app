...
from app.schemas.procurement import (BidCreate, InventoryItemCreate,
                                     InventoryItemOut, PurchaseOrderCreate,
                                     PurchaseOrderOut, VendorCreate, VendorOut)
...
def update_vendor(
    ...
    updated = ProcurementService.update_vendor(
        db, vendor_id, current_user.company_id, vendor
    )
    if not updated:
...
def create_purchase_order(
    ...
    return ProcurementService.create_purchase_order(
        db, po, current_user.id, current_user.company_id
    )
...
def approve_purchase_order(
    ...
    if current_user.role not in ["COMPANY_ADMIN", "SUPERADMIN"]:
        raise HTTPException(
            status_code=403, detail="Only admins can approve purchase orders"
        )
    approved = ProcurementService.approve_purchase_order(
        db, po_id, current_user, current_user.company_id
    )
    if not approved:
        raise HTTPException(
            status_code=404, detail="Purchase order not found or not pending"
        )
    # Audit log
...
def reject_purchase_order(
    ...
    if current_user.role not in ["COMPANY_ADMIN", "SUPERADMIN"]:
        raise HTTPException(
            status_code=403, detail="Only admins can reject purchase orders"
        )
...
def add_bid(
    ...
    updated = ProcurementService.add_bid_to_purchase_order(
        db, po_id, bid, current_user.company_id
    )
    if not updated:
        raise HTTPException(status_code=404, detail="Purchase order not found")
    return updated
...
async def get_inventory(
    ...
    return await ProcurementService.get_inventory_items(
        db, current_user.company_id, name
    )


@router.post(
    "/inventory/", response_model=InventoryItemOut, status_code=status.HTTP_201_CREATED
)
async def create_inventory_item(
    ...
    return await ProcurementService.create_inventory_item(
        db, item, current_user.company_id
    )
...
async def update_inventory_item(
    ...
    updated = await ProcurementService.update_inventory_item(
        db, item_id, current_user.company_id, item
    )
    if not updated:
...
async def delete_inventory_item(
    ...
    if not await ProcurementService.delete_inventory_item(
