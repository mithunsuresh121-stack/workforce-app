...
from app.crud import (create_shift, delete_shift, get_shift_by_id,
                      list_shifts_by_company, list_shifts_by_employee,
                      update_shift)
...
def get_shift(
    ...
    if (
        current_user.role != "SuperAdmin"
        and shift.company_id != current_user.company_id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def delete_shift_schedule(
    ...
    if current_user.role != "SuperAdmin" and shift.tenant_id != str(
...
@router.post(
    "/swap-request/", response_model=SwapRequestOut, status_code=status.HTTP_201_CREATED
)
def request_shift_swap(
    ...
    if not requester_shift:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Requester shift not found"
        )
    ...
    if not target_shift:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Target shift not found"
        )
    ...
    if (
        requester_shift.start_at < datetime.utcnow()
        or target_shift.start_at < datetime.utcnow()
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
def approve_shift_swap(
    ...
    """
    swap_request = (
        db.query(SwapRequest).filter(SwapRequest.id == swap_request_id).first()
    )
    if not swap_request:
        raise HTTPException(
    ...
    can_approve = (
        swap_request.target_employee_id == current_user.id
        or current_user.role in ["SuperAdmin", "CompanyAdmin", "Manager"]
    )
    ...
    if not requester_shift or not target_shift:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="One or both shifts not found"
        )
