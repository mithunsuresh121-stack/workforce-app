...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, room_type: str, room_id: int, user: User, db: Session):
        ...
        try:
            ...
            self.disconnect(websocket, room_type, room_id, user.id)
    ...
    def disconnect(self, websocket: WebSocket, room_type: str, room_id: int, user_id: int):
        ...
        # Set offline
        # Note: company_id needs to be passed or stored
        # For simplicity, assume company_id=1; in production, store in connection metadata
        redis_service.set_user_offline(1, user_id)  # TODO: Pass company_id properly
    ...
    async def handle_message(self, websocket: WebSocket, data: dict, room_type: str, room_id: int, user: User, db: Session):
        ...
        if room_type == "chat":
            if msg_type == "typing":
                await chat_service.set_typing_indicator(room_id, user.id)
                await self.broadcast(websocket, {"type": "typing", "user_id": user.id, "channel_id": room_id}, room_type, room_id, user.id)
                ...
                pass  # Delegate to service and broadcast
            else:
                logger.warning("Unknown chat message type", type=msg_type)

        elif room_type == "meeting":
            if msg_type in ["offer", "answer", "ice-candidate"]:
                await self.broadcast(websocket, {"type": msg_type, "from": user.id, "data": data.get("data")}, room_type, room_id, user.id)
            elif msg_type == "presence":
                await meeting_service.get_online_participants(room_id, user.company_id)
                await self.broadcast(websocket, {"type": "presence_update", "online_users": []}, room_type, room_id, user.id)  # Fetch and broadcast
            else:
                logger.warning("Unknown meeting message type", type=msg_type)
