...
for workflow in backend-tests frontend-tests mobile-tests; do
  run_numbers=$(gh run list --workflow "$workflow" --json number | jq -r '.[] .number')
  for num in $run_numbers; do
    gh run delete "$num"
  done
done
...
for status in failure cancelled skipped; do
  run_numbers=$(gh run list --workflow build-apk.yml --status "$status" --json number | jq -r '.[] .number')
  for num in $run_numbers; do
    gh run delete "$num"
  done
done
...
successful_runs=$(gh run list --workflow build-apk.yml --status success --json number,createdAt | jq -r 'sort_by(.createdAt) | reverse | .[3:] | .[] .number')
for num in $successful_runs; do
  gh run delete "$num"
done
